...
- ◐ **White-label depth:** email/notification templates now wired into live sends (Month-1 per-tenant-branding fix landed 2026-08-23); **logo image upload shipped 2026-08-25** — `POST/DELETE /api/admin/tenants/[id]/logo`, admin-only, png/jpeg/webp/svg ≤512KB, stored as a data-URL in `branding.logoUrl` (documented fallback: no Supabase Storage bucket precedent exists in the codebase yet — switch to real Storage if one gets introduced later). Still missing: custom login page, KB links, privacy policy, **secondary brand color field** (only `primary`/`sidebarBg`/`primaryForeground` exist), **support phone field** (`supportEmail` exists on `TenantBranding`, `supportPhone` does not)
