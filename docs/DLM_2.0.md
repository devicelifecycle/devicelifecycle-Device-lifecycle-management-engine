...
- ☐ **NEW (2026-08-17 audit): per-tenant branding on outbound emails + PDFs.** `email.service.ts` / `pdf.ts` / `rve-pdf.ts` currently use one global `APP_NAME` env var for every tenant. Needs each send/generate call to resolve the order's tenant branding (`resolveBranding` already exists — reuse it) instead of the global constant. Highest-priority white-label gap found in the audit.
...
**Customer Data Management** (import/export/search/reports/history) — ◐ order-level import/export/search exist; ☐ no bulk import for the *asset* register specifically (API is single-row only); ☐ asset change history (same as "Audit" gap in Month 2).

**Customer Devices/Assets** (register/assign/retire/move/audit/track) — ◐ shipped 2026-08-17 except "audit" (change-history log, ☐, Month 2 new item).

**Customer Reports** (operational/compliance/inventory/financial/user-activity/device-history/exports) — ☐ not built, matches the already-tracked "own reports/exports" bullet in M2.5.
