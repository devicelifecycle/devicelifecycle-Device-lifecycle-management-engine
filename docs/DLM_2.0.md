...
- ◐ **Auth enforcement** — landed 2026-08-27 (concurrent work stream, verified against live code): **IP allowlist is genuinely enforced end-to-end** — `tenants.branding.allowedIps` is settable from `admin/tenants/[id]`, and `requireAuth()` rejects non-matching requests. `src/lib/password-policy.ts` exists but **has zero callers anywhere in the codebase** — registration/password-change routes don't call it, so password policy is not actually enforced despite the file existing. SSO and MFA are still ☐ not started.
