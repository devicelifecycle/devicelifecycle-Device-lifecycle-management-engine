...
- ◐ **Integrations** — API key **management** landed 2026-08-27 (`GET/POST /api/var/api-keys`, `DELETE .../[id]`, hashed storage, VAR + admin UI at `/var/api-keys`) — found and fixed a real auth gap (GET had no role check) and a stale route-params bug during review. **But there is no public API yet** — `requireApiKey()` (the bearer-token auth guard) is fully built but **guards zero actual routes**; a generated key currently unlocks nothing. SMS/SMTP sender config landed as `/var/communications` (emailFromName/emailFromAddress/smsSenderId on tenant branding) but **isn't consumed by the actual send path yet** — `email.service.ts`'s `getFromEmail()` still only reads the global env var. Payment gateway still ☐, deferred to the client face-to-face per Open decisions.
- ◐ **White-label depth:** email/notification templates wired into live sends (2026-08-23); logo image upload shipped (2026-08-25). Still missing: custom login page, KB links on the branding side, privacy policy customization, secondary brand color field, support phone field.
...
> **Month 4 status as of 2026-08-27:** impersonation, IP allowlist, ticket SLAs, and the knowledge base are genuinely done end-to-end. API key *management* and VAR communications *settings* are built but not yet consumed by anything (no public API route checks a key; no email actually sends from a custom sender identity yet). The granular RBAC model and password-policy enforcement are the two most overstated-risk items — both have real code sitting in the tree that a status report could mistake for "shipped," but neither is actually wired into a single real enforcement point yet. Remaining before Month 4 is closed: SSO, MFA, wire `requirePermission`/`useCan` into real routes (or explicitly decide this stays role-based), call `password-policy.ts` from registration/password-change, pick a public-API surface for the API keys to actually guard, wire `getFromEmail()` to the new per-tenant sender fields, secondary brand color + support phone + login page + KB-link branding, live chat, retention policy, and full launch QA.
