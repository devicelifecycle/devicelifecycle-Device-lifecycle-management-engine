...
**BB Admin Security** (roles, permissions, SSO, MFA, API keys, encryption keys, audit logs, IP restrictions, password policy, retention policies) — ◐ roles/audit shipped; IP restrictions ✅ genuinely enforced (2026-08-27); API keys ◐ management UI shipped but no public API consumes them yet; permissions ◐ catalog/guard infra exists but zero routes actually use it; password policy ☐ file exists, zero callers; ☐ SSO, MFA, retention policies; ❓ encryption keys (Month 4 new item).
