...
function setImpl(impl: 'ts' | 'scrapling') {
  process.env.SCRAPER_GORECELL_IMPL = impl
}
...
async function main() {
  console.log('Validating GoRecell targeted sample devices...')
  setImpl('ts')
  const tsTargeted = await scrapeGoRecell([...commonValidationDevices])
  setImpl('scrapling')
  const scraplingTargeted = await scrapeGoRecell([...commonValidationDevices])

  console.log('Validating GoRecell discovery mode...')
  setImpl('ts')
  const tsDiscovery = await scrapeGoRecellFullCatalog()
  setImpl('scrapling')
  const scraplingDiscovery = await scrapeGoRecellFullCatalog()
  ...
  console.log('\nGoRecell Scrapling validation passed.')
}
