#!/usr/bin/env npx tsx
/**
 * Validate the UniverCell Scrapling pilot against the existing TS scraper.
 *
 * Usage:
...
import { univercellValidationDevices } from '../src/lib/scrapers/validation-fixtures'
...
function setImpl(impl: 'ts' | 'scrapling') {
  process.env.SCRAPER_UNIVERCELL_IMPL = impl
}
...
async function main() {
  console.log('Validating targeted sample devices...')
  setImpl('ts')
  const tsTargeted = await scrapeUniversal([...univercellValidationDevices])
  setImpl('scrapling')
  const scraplingTargeted = await scrapeUniversal([...univercellValidationDevices])

  console.log('Validating discovery mode...')
  setImpl('ts')
  const tsDiscovery = await scrapeUniversalFullCatalog()
  setImpl('scrapling')
  const scraplingDiscovery = await scrapeUniversalFullCatalog()
  ...
  console.log('\nUniverCell Scrapling validation passed.')
}
