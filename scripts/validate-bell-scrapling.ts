...
function setImpl(impl: 'ts' | 'scrapling') {
  process.env.SCRAPER_BELL_IMPL = impl
}
...
async function main() {
  console.log('Validating Bell targeted sample devices...')
  setImpl('ts')
  const tsTargeted = await scrapeBell([...commonValidationDevices])
  setImpl('scrapling')
  const scraplingTargeted = await scrapeBell([...commonValidationDevices])

  console.log('Validating Bell discovery mode...')
  setImpl('ts')
  const tsDiscovery = await scrapeBellFullCatalog()
  setImpl('scrapling')
  const scraplingDiscovery = await scrapeBellFullCatalog()
  ...
  console.log('\nBell Scrapling validation passed.')
}
