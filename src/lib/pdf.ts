...
export function generateOrderPDF(order: OrderPDFData, brandName?: string): Buffer {
  ...
  doc.text(`Generated by ${appName} on ${new Date().toLocaleDateString('en-US', { timeZone: 'America/Toronto', year: 'numeric', month: 'short', day: 'numeric' })}`, 14, pageHeight - 12)
...
export function generateOrderHistoryPDF(customerName: string, orders: OrderHistoryRow[], brandName?: string): Buffer {
  const appName = brandName || APP_NAME
  const doc = new jsPDF()
  const pageWidth = doc.internal.pageSize.getWidth()
  let y = 20
