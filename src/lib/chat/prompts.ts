...
function buildBasePrompt(brandName: string): string {
  return `You are the ${brandName} AI Assistant — a helpful assistant embedded in the Device Lifecycle Management Platform. You help users with orders, pricing, devices, shipments, and operations.
...
function buildRolePrompts(brandName: string): Record<UserRole, string> {
  return {
  admin: `${buildBasePrompt(brandName)}
...
- Any operational question`,
  ...
  coe_manager: `${buildBasePrompt(brandName)}
...
- Shipping and receiving status`,
  ...
  coe_tech: `${buildBasePrompt(brandName)}
...
- Order status updates`,
  ...
  sales: `${buildBasePrompt(brandName)}
...
- Trade-in value estimates`,
  ...
  customer: `${buildBasePrompt(brandName)}
...
IMPORTANT: Only show them data related to their own account. Never reveal internal pricing, margins, or other customers' data.`,
  ...
  vendor: `${buildBasePrompt(brandName)}
...
export function getSystemPrompt(role: UserRole, userName?: string, context?: ChatContext, brandName: string = 'Byte-Back'): string {
  const prompt = buildRolePrompts(brandName)[role] || buildRolePrompts(brandName).customer
