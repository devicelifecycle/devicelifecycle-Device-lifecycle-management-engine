...
const COMPETITOR_NAME = 'Bell'
const DEFAULT_TARGETED_TIMEOUT_MS = 90_000
const DEFAULT_DISCOVERY_TIMEOUT_MS = 240_000
...
export function getBellScraperImpl(): BellScraperImpl {
  const raw = (process.env.SCRAPER_BELL_IMPL || 'ts').trim().toLowerCase()
  if (raw === 'ts' || raw === 'scrapling' || raw === 'dual') return raw
  return 'ts'
}

function getWorkerScriptPath(): string {
  return path.join(process.cwd(), 'scrapers_py', 'bell_worker.py')
}
