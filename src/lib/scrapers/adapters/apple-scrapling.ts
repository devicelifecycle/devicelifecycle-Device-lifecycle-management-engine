...
export type AppleScraperImpl = 'ts' | 'scrapling' | 'dual'

const COMPETITOR_NAME = 'Apple Trade-In'
...
export function getAppleScraperImpl(): AppleScraperImpl {
  const raw = (process.env.SCRAPER_APPLE_IMPL || 'ts').trim().toLowerCase()
  if (raw === 'ts' || raw === 'scrapling' || raw === 'dual') return raw
  return 'ts'
}

function getWorkerScriptPath(): string {
  return path.join(process.cwd(), 'scrapers_py', 'apple_worker.py')
}
...
export async function runAppleScraplingWorker(devices: DeviceToScrape[]): Promise<ScraperResult> {
  ...
  return new Promise((resolve) => {
    ...
    child.stdin.write(JSON.stringify({ mode: 'targeted', devices }))
    child.stdin.end()
  })
}

export async function runAppleScraperPilot(options: {
  devices: DeviceToScrape[]
  runTypeScript: () => Promise<ScraperResult>
}): Promise<ScraperResult> {
  const impl = getAppleScraperImpl()

  if (impl === 'ts') return options.runTypeScript()
  if (impl === 'scrapling') return runAppleScraplingWorker(options.devices)
