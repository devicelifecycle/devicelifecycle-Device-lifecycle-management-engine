...
export type TelusScraperImpl = 'ts' | 'scrapling' | 'dual'

const COMPETITOR_NAME = 'Telus'
...
export function getTelusScraperImpl(): TelusScraperImpl {
  const raw = (process.env.SCRAPER_TELUS_IMPL || 'ts').trim().toLowerCase()
  if (raw === 'ts' || raw === 'scrapling' || raw === 'dual') return raw
  return 'ts'
}

function getWorkerScriptPath(): string {
  return path.join(process.cwd(), 'scrapers_py', 'telus_worker.py')
}

function getPythonBin(): string {
  if (process.env.SCRAPLING_PYTHON_BIN) return process.env.SCRAPLING_PYTHON_BIN
  return path.join(process.cwd(), '.venv-scrapling', 'bin', 'python')
}
...
export async function runTelusScraplingWorker(devices: DeviceToScrape[], discovery = false): Promise<ScraperResult> {
  ...
  return new Promise((resolve) => {
    ...
    child.stdin.write(JSON.stringify({ mode: discovery ? 'discovery' : 'targeted', devices }))
    child.stdin.end()
  })
}

export async function runTelusScraperPilot(options: {
  devices: DeviceToScrape[]
  discovery?: boolean
  runTypeScript: () => Promise<ScraperResult>
}): Promise<ScraperResult> {
  const impl = getTelusScraperImpl()
  const discovery = options.discovery === true

  if (impl === 'ts') return options.runTypeScript()
  if (impl === 'scrapling') return runTelusScraplingWorker(options.devices, discovery)

  const [tsResult, scraplingResult] = await Promise.all([
    options.runTypeScript(),
    runTelusScraplingWorker(options.devices, discovery),
  ])
